...
class Location_ControllerAzure(LocationControllerAbstract):

    def __init__(self, serializer: Serializer=None):
        super().__init__(serializer, database)
        self._LOCATION_STRING = "[dbo].[location_combinations]"
    ...
    def get_next_location_combinations(self, amount: int = 1, min_time_delta: timedelta = timedelta(0)) -> df:
        ...
        data = self._serializer.read(f'SELECT TOP {amount} * FROM {self._LOCATION_STRING} \
